...
- **Person（乘客）**
  ...
  - assignedElevatorId: 分配到的電梯 (可選，`ElevatorId | undefined`)
  - pickupTime: 進入時間 (可選，`number | undefined`)
  - dropOffTime: 離開時間 (可選，`number | undefined`)
...
#### 動態目標調整 (Dynamic Target Adjustment - LOOK)：

- 電梯不會盲目地運行到最高或最低樓層 (傳統 SCAN 的缺點)。
- LOOK 根據當前方向上最遠的請求（乘客目的地或樓層呼叫）來決定轉向點。
...
- **乘客重新分配**：如果電梯在停靠時已滿，無法接納等待的乘客，這些乘客的 `assignedElevatorId` 會被清除，其原始呼叫會被重新加入樓層呼叫佇列 (`floorCalls`)，等待下一次分配機會，確保他們最終能被服務。
...
- 效率提升：相較於簡單的先來先服務，LOOK 策略能更有效地組織電梯路徑，減少空跑和乘客平均等待時間。
...
- **避免無窮等待**：
  當電梯滿載而無法服務某樓層的等待乘客時，這些乘客的請求會被重新加入待分配佇列，確保所有乘客最終都能完成旅程。
- **日誌與異常偵測**：
  模擬過程中記錄極為詳細的日誌（包括電梯選擇的評分過程、狀態變更、乘客上下等），有助於追蹤和分析演算法行為。若有乘客在模擬結束時仍未完成，系統會發出警告並記錄相關統計數據，方便進行除錯與演算法優化。

---

### 未來改進事項

- 實驗更多不同的電梯調度策略組合，以進一步提升整體運行效率和乘客滿意度。
- 目前模擬參數雖然可以透過修改 `src/constants/simulationConfig.ts` 檔案來調整，但未來可以考慮在 UI 上提供動態調整這些參數（如電梯數量、容量、乘客產生速率等）的功能，以增加模擬的靈活性和互動性，使其更能模擬多種現實情況。
- 增加更複雜的乘客行為模式，例如乘客中途改變目的地或取消呼叫。
